...
def remove_and_redirect(request, removable, message_detail):
    ...
    if getattr(settings, "EDIT_REMOVE_ENABLED", True):
        ...
        removable.save()
        class_name = (
            removable._meta.verbose_name.title()
        )  # pylint: disable=protected-access
        class_ = class_name[:1].lower() + class_name[1:] if class_name else ""
        messages.success(
            request,
            _("Removed {object_type}: {detail}").format(
                object_type=class_, detail=message_detail
            ),
        )  # nopep8
        return HttpResponseRedirect(
            reverse("openach:detail", args=(removable.board.id,))
        )
    else:
