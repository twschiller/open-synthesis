...
class ProfileTests(PrimaryUserTestCase):
    ...
    def _add_hypothesis(self, user=None):
        self.hypothesis = Hypothesis.objects.create(
            hypothesis_text="Hypothesis", creator=user, board=self.board
        )

    def _add_evidence(self, user=None):
        self.evidence = Evidence.objects.create(
            evidence_desc="Evidence", creator=user, board=self.board
        )
    ...
    def test_update_settings(self):
        ...
        self.assertEqual(
            self.user.settings.digest_frequency, DigestFrequency.weekly.key
        )
