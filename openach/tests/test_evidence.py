...
from .common import PrimaryUserTestCase, add_follower, create_board, remove
...
class AddEvidenceTests(PrimaryUserTestCase):
    ...
    def test_require_login_for_add_evidence(self):
        ...
        response = self.client.get(
            reverse("openach:add_evidence", args=(self.board.id,))
        )
    ...
    def test_can_show_edit_form(self):
        ...
        self.login()
        response = self.client.get(
            reverse("openach:edit_evidence", args=(self.evidence.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_submit_edit_form(self):
        ...
        self.assertGreaterEqual(
            len(Evidence.objects.filter(evidence_desc="Updated Evidence Description")),
            1,
        )
...
class EvidenceDetailTests(PrimaryUserTestCase):
    def setUp(self):
        ...
        self.tags = [
            EvidenceSourceTag.objects.create(
                tag_name="Tag #1", tag_desc="Description for Tag #1"
            ),
            EvidenceSourceTag.objects.create(
                tag_name="Tag #2", tag_desc="Description for Tag #2"
            ),
        ]
    ...
    def test_evidence_detail_view(self):
        ...
        response = self.client.get(
            reverse("openach:evidence_detail", args=(self.evidence.id,))
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_source_tag(self):
        ...
        self.login()
        response = self.client.post(
            reverse("openach:tag_source", args=(self.evidence.id, self.source.id)),
            data={"tag": self.tags[0].tag_name},
        )
        self.assertEqual(response.status_code, 302)
