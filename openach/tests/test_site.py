...
class IndexViewTests(TestCase):
    def test_can_access_request_context(self):
        ...
        self.assertIsNotNone(
            response.context, "Context was not returned with index view response"
        )
    ...
    def test_do_not_show_future_news(self):
        ...
        ProjectNews.objects.create(
            content="Test news", pub_date=timezone.now() + datetime.timedelta(days=5)
        )
        response = self.client.get(reverse("openach:index"))
        self.assertNotContains(response, "Project News")
    ...
    def test_show_published_news(self):
        ...
        ProjectNews.objects.create(
            content="Test news", pub_date=timezone.now() + datetime.timedelta(days=-1)
        )
        response = self.client.get(reverse("openach:index"))
...
class AboutViewTests(TestCase):
    ...
    def test_can_create_bitcoin_donation_link(self):
        ...
        self.assertIn(
            "abc123", bitcoin_donation_url(site_name="anything", address=self.address)
        )
