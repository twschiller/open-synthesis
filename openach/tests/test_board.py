...
class BoardFormTests(PrimaryUserTestCase):
    ...
    def test_submit_valid_create_board_long_title(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertGreater(
            len(Board.objects.filter(board_slug="x" * SLUG_MAX_LENGTH)), 0
        )
    ...
    def test_can_show_edit_form(self):
        ...
        board = Board.objects.create(
            board_title="Board #1", creator=self.user, pub_date=timezone.now()
        )
    ...
    def test_board_meta_edit_permissions(self):
        ...
        board = Board.objects.create(
            board_title="Board #1", creator=None, pub_date=timezone.now()
        )
        self.login()
        response = self.client.get(reverse("openach:edit_board", args=(board.id,)))
    ...
    def test_can_submit_edit_form(self):
        ...
        response = self.client.post(
            reverse("openach:edit_board", args=(board.id,)),
            data={
                "board_title": "New Board Title",
                "board_desc": "New Board Description",
            },
        ...
        self.assertGreaterEqual(
            len(Board.objects.filter(board_title="New Board Title")), 1
        )
        self.assertGreaterEqual(
            len(Board.objects.filter(board_desc="New Board Description")), 1
        )

        # check that field history was recorded
        self.assertEqual(
            FieldHistory.objects.get_for_model_and_field(board, "board_title").count(),
            2,
        )
        self.assertEqual(
            FieldHistory.objects.get_for_model_and_field(board, "board_desc").count(), 2
        )
    ...
    def test_can_remove_board(self):
        ...
        self.login()
        response = self.client.post(
            reverse("openach:edit_board", args=(board.id,)), data={"remove": "remove"}
        )
        self.assertEqual(response.status_code, 302)
...
class BoardDetailTests(PrimaryUserTestCase):
    ...
    def test_can_display_disagreement_with_no_assessments(self):
        ...
        response = self.client.get(
            reverse("openach:detail", args=(self.board.id,)) + "?view_type=disagreement"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_display_comparison_no_assessments(self):
        ...
        self.login()
        response = self.client.get(
            reverse("openach:detail", args=(self.board.id,)) + "?view_type=comparison"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_order_hypotheses_and_evidence(self):
        ...
        def mk_evidence(desc):
            return Evidence.objects.create(
                board=self.board, creator=self.user, evidence_desc=desc, event_date=None
            )
        ...
        self.assertGreater(
            len(
                [
                    scored
                    for scored in response.context["evidences"]
                    if scored[1][0] < 0.0
                ]
            ),
            0,
        ...
        self.assertGreater(
            len(
                [
                    scored
                    for scored in response.context["hypotheses"]
                    if scored[1][0] > 0.0
                ]
            ),
            0,
        ...
        self.assertEqual(
            response.context["evidences"][0][0],
            diagnostic,
        ...
        self.assertEqual(
            response.context["hypotheses"][0][0],
            self.hypotheses[1],
