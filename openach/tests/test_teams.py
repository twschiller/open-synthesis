...
from .common import PrimaryUserTestCase, HTTP_OK, HTTP_FORBIDDEN, HTTP_REDIRECT
...
class TeamTests(PrimaryUserTestCase):
    ...
    def test_edit_team(self):
        ...
        response = self.client.post(reverse('openach:edit_team', args=(self.team.id,)), data={
            'name': 'Team',
            'description': desc,
            'public': False,
        })
    ...
    def test_can_create_team_post(self):
        ...
        response = self.client.post(reverse('openach:create_team'), data={
            'name': team_name,
...
class TeamMembership(PrimaryUserTestCase):
    ...
    def test_user_can_join_directly(self):
        ...
        self.assertFalse(TeamRequest.objects.filter(invitee=self.user, team=self.team).exists())
    ...
    def test_user_can_request_invite(self):
        ...
        self.team = Team.objects.create(name='Test Team', owner=self.other, invitation_required=True)
    ...
    def test_user_can_accept_invitation(self):
        ...
        TeamRequest.objects.create(inviter=self.other, invitee=self.user, team=self.team)
    ...
    def test_can_revoke_membership(self):
        ...
        self.team = Team.objects.create(name='Test Team', invitation_required=True, owner=self.user)
        self.team.members.add(self.other)
        self.team.save()

        self.login()

        response = self.client.post(reverse('openach:revoke_membership', args=(self.team.id, self.other.id)))
        self.assertStatus(response, HTTP_REDIRECT)
    ...
    def test_can_accept_membership_request(self):
        ...
        membership_request = TeamRequest.objects.create(invitee=self.other, team=self.team)

        response = self.client.post(reverse('openach:decide_invitation', args=(membership_request.id,)), data={
            'accept': True,
        })
        self.assertStatus(response, HTTP_REDIRECT)
    ...
    def test_can_invite_members(self):
        ...
        response = self.client.get(reverse('openach:invite_members', args=(self.team.id,)))
        self.assertTemplateUsed(response, 'teams/invite.html')

        response = self.client.post(reverse('openach:invite_members', args=(self.team.id,)), data={
